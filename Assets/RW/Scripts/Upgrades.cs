...
public class Upgrades : MonoBehaviour
{
    ...
    private static int totPoints = 0; // change avaliable points here for debug purposes [GODMODE]

    public void goBack()
    {
        SceneManager.LoadScene(0);

        // exports for next creation of Upgrades.
        Upgrades_params.setinShootRate(inShootRate); 
        Upgrades_params.setinSheepRate(inSheepRate);
        Upgrades_params.setinMovSpeed(inMovSpeed);
    }
    ...
    public void sheep()
    {
        ...
        {
            Debug.Log("Upgrade 2");
            sheepBar[inSheepRate].SetActive(true);
            inSheepRate++;
            SheepSpawner.spawnTime(0.1F); // makes us shoot faster
            totPoints -= 5;
            txt.text = "Points: " + totPoints.ToString();
    ...
    public void mov()
    {
        ...
        if (inMovSpeed < 5 && totPoints >= 5) // if the stat is not already maxed and the player has points to buy it
        {
            ...
            inMovSpeed++;
            HayMachine.movementSpeedPlayer(0.1F); // makes us shoot faster
            totPoints -= 5;
            txt.text = "Points: " + totPoints.ToString();
